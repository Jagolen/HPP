...
void create_tree(qt *head,double min_x, double min_y, double max_x, double max_y, vec *particles, int N){
    ...
    if(numparticles > 1){//If there are more than one particle in a region, we have to create sub branches
        ...
        //Split the region into four squares with one sub tree per square
...
void delete_tree(qt **head){
    ...
    //Otherwise we delete the nodes recursively.
...
int main(const int argc, char *argv[]){
    ...
    if (argc != 7) {
        printf("Give 5 input arguments: int N, filename.gal , int nsteps, double delta_t, double theta, graphics (0 or 1) .\n");
        exit(1);
    }
